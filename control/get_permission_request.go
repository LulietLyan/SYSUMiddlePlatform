...
func GetPermissionRequest(c *gin.Context) {
	type msg struct {
	...
	if e := c.ShouldBindQuery(&m); e == nil {
		...
		if e := mysql.DB.Raw(`
		SELECT T1.U_username,T1.PT_name,T1.PR_uid,T1.PR_level,T1.updated_at,User.U_username requester_pu_name FROM
			(SELECT User.U_username,ProjectTable.PT_name,PermissionRequest.PR_uid,PermissionRequest.PR_level,PermissionRequest.PU_uid,PermissionRequest.updated_at
				...
				ORDER BY PermissionRequest.updated_at DESC
				LIMIT ?
				OFFSET ?
			)T1,User,ProjectUser
		WHERE T1.PU_uid=ProjectUser.PU_uid AND ProjectUser.U_uid=User.U_uid
	`, m.Limit, m.Offset).Scan(&records).Error; e != nil {
			response.Fail(c, nil, "")
			return
		}
		type message struct {
